...
export const HELP_ARTICLES: HelpArticle[] = [
  ...
  {
    id: "stores-items", category: "Stores", screens: ["/inventory"],
    title: "Items — what you buy and keep",
    summary: "Set up the material you hold in the store, and the unit each one is measured in.",
    body: "An item is anything you buy. Cable, conduit, glands, consumables. Not all of it is stocked: hire, a service, or a one-off bought and used the same day never sits on a shelf, so it is marked as going straight to cost and charged to the job on a supplier invoice instead. Marking that correctly matters, because a non-stocked item can never appear on a stock report pretending to be there.",
    steps: [
      "Open Stores -> Items and click Add item.",
      "Give it a short code people will actually type, like CBL-4C-16, and say what it is.",
      "Choose the unit. It is the same list invoices use, so a metre bought and a metre billed mean the same thing.",
      "Leave 'Keep stock of this' ticked for anything that sits on a shelf. Untick it for hire and services.",
      "Set a reorder level if somebody should be chasing it. Leave it at nought if nobody does.",
    ],
    tip: "Standard cost is for estimating only. Stock is always valued at what was actually paid, never at the standard.",
  },
  {
    id: "stores-stock", category: "Stores", screens: ["/inventory/stock"],
    title: "Stock on hand, and how it is valued",
    summary: "What is on the shelf right now, what it is worth, and why the average moves.",
    body: "Stock is valued at weighted average. Cable bought at three prices over a month is the same cable on the drum, and no storekeeper can tell you which metre came from which delivery. So receiving at a new price moves the average of everything already there, and issuing takes material out at whatever the average is at that moment without changing it. Every figure on this screen is added up from the movements themselves rather than read from a stored number, so the report and its own history can never disagree.",
    steps: [
      "Open Stores -> Stock on Hand.",
      "Read the sentence at the top. It leads with anything showing a negative balance, because that means the record and the shelf have already parted company.",
      "Use the store links to look at one store, or all of them together.",
      "Anything at or below its reorder level is highlighted.",
      "Show empty brings in the items that are set up but have nothing on hand.",
    ],
    tip: "A negative balance is never normal. It means something was issued that was never received, so count the shelf and put the difference through as an adjustment before trusting any other figure.",
  },
  {
    id: "stores-movements", category: "Stores", screens: ["/inventory/movements"],
    title: "Receiving and issuing material",
    summary: "Recording what arrived, what went to a job, and what that does to the accounts.",
    body: "Material arriving is an asset, not a cost. It only becomes a cost on the day it is issued to a job, which is why an issue has to name one. Getting this the wrong way round is what makes a month look terrible because a delivery landed on the 30th, and the month after look wonderful for the same reason. An issue is priced from the shelf automatically, so there is nothing to type and nothing to get wrong.",
    steps: [
      "Open Stores -> Receive & Issue and click Record a movement.",
      "Say what happened. The form then asks only for what that kind needs.",
      "Material arrived: give the quantity, what one unit cost, and the delivery note.",
      "Material out to a job: give the quantity and choose the job. The price comes from the shelf.",
      "Moved between our stores: choose the second store. Nothing is posted, because what the company owns has not changed.",
    ],
  ...
  {
    id: "settings-email", category: "Administration & Security", screens: ["/settings/email"],
    title: "Connecting your mail server",
    summary: "Set it up once and quotations go to customers straight from the system.",
    body: "Nothing about your mail server is written into the application. The server name, the port, how the connection is secured and which mailbox it signs in as are all set on this screen, per company — a group sends as White and Bright Engineering or as WB Technical Services depending on whose work it is, and the customer should see the right one. Changing provider later is a change here rather than a change to the software. For Microsoft 365 the settings that catch people out are the server, which is smtp.office365.com and not outlook.com, and the port, which is 587 with STARTTLS rather than 465. Microsoft also turns SMTP AUTH off by default on newer tenants, so your IT will need to enable it for the mailbox you are using, and where multi-factor authentication is on you will need an app password rather than the normal one. The password is encrypted before it is stored and is never shown back, not even to an administrator — it is a way into the mailbox where every contract, invoice and price the company has ever sent lives. Leaving the password box empty when you change something else keeps the saved one; clearing it is a separate tick.",
    steps: [
      "Open Settings -> Email and choose your provider. Microsoft 365 fills in the server, port and security for you.",
      "Enter the mailbox to sign in as — the full address — and its password or app password.",
      "Set the name and address quotations should come from, and a reply-to if replies should go somewhere else.",
      "Save, then click Send a test. It goes to you, not to a customer: finding out the server refuses us by sending rubbish to a client is an expensive way to learn.",
      "If the test fails, the message underneath is what your mail server actually said — that is the line to give your IT.",
      "Once it works, the Issue button on a quotation sends it, and every send is listed at the bottom of this screen.",
    ],
  ...
  },
  {
    ...
    body: "A customer is not one person. A quotation usually goes to procurement with the project manager copied, and the invoice goes somewhere else entirely. Adding them here means the quotation screen offers them as tick boxes when it goes out, which is the point: an address chosen from a list is right, and one typed from memory loses a letter and nobody notices for a week. One of them is the main contact, which is the one ticked by default.",
    steps: [
      "Open Finance -> Parties and click Add in the Contacts column of the customer.",
      "Enter their name, what they do, and their email address.",
      "Tick the main contact — the one a quotation defaults to. Only one can be the main one.",
      "When somebody who has left is no longer there, untick 'Still there' rather than removing them, so old quotations still read.",
      "The contacts then appear on the quotation issue dialog, and whoever is sending ticks who it goes to.",
    ],
    tip: "An address that is not on file can still be typed into the quotation. Add it here afterwards so the next person does not have to.",
  },
  {
    ...
    steps: [
      ...
      "Once signed, click Email it to the customer, tick who it goes to from the customer's contacts, and send. The quotation PDF on your letterhead is attached. If no mail server has been set up in Settings -> Email, the button says Issue to the customer instead: download the PDF, send it from your own mailbox, and record where it went.",
  ...
  },
  {
    ...
    body: "Every contractor's sales report says there is forty million in the pipeline, and the figure is usually fiction for three reasons. This one prevents all three. Won and lost enquiries leave the pipeline entirely: a won deal is revenue and belongs on a job, a lost one is nothing, and a forecast that keeps either only ever goes up. The probability of winning comes from the stage an enquiry has actually reached, never from a box somebody fills in — typed per deal, every deal becomes ninety per cent and the weighted total is just the gross total with extra steps. And the headline figure is the weighted one, with the gross beside it in smaller type, because the gross is the number people quote and the weighted one is the number that is true. The board shows a column per stage so you can see what is stuck where, and the bar chart above it is drawn against the biggest stage rather than the first, so the longest bar is the blockage — forty enquiries sitting at Estimating because nobody has priced them.",
    steps: [
      "Open CRM and Estimation. Each column is a stage, with its own weighted total underneath.",
      "Click Log an enquiry when a customer asks for a price. Only the title and who it is from are needed — the rest can follow.",
      "Open any card to see where it has got to, on a bar like a parcel tracker.",
      "Use Move it on as it progresses. The dialog shows what the move does to the chance of winning before you commit to it.",
      "Losing asks why, and who won it. That is the only moment anybody still knows.",
      "Switch to All to see everything decided, with how far each one got before it went.",
    ],
    tip: "The five lamps on each card count what is KNOWN about the enquiry — budget, decision-maker, date needed, site visited, scope defined. Few lamps lit is not a bad enquiry; it is one nobody has done the work on yet, which is something you can actually act on.",
  },
  {
    id: "crm-site-visit", category: "CRM & Estimation", screens: ["/crm"],
    title: "Site visits and the report",
    summary: "The visit and the report are two events, and the status only follows the second.",
    body: "Somebody goes on Tuesday and writes it up on Friday, so recording a visit and filing its report are two separate things. The enquiry shows Site Report Submitted only once a report actually exists — never when a visit has merely been booked. A status somebody ticks is a status that is wrong the day a visit slips, and the report is what the whole estimate gets built on, so claiming one exists when it does not is the expensive kind of wrong.",
    steps: [
      "Open the enquiry and click Record beside Site visits.",
      "Fill in the date and who is going. Leave the findings empty if the visit has not happened yet — the note at the bottom tells you which way it will save.",
      "The enquiry then shows the visit as booked and the report as still to come.",
      "After the visit, click File the report and write what was found.",
      "Only then does the enquiry show Site Report Submitted, and only then does that count towards what is known about it.",
    ],
    tip: "An empty report is refused. It would mark the enquiry as reported when nothing was, which is worse than no report at all.",
  },
  {
  ...
  {
    id: "stores-requests", category: "Stores", screens: ["/inventory/requests"],
    title: "Asking for material: the material request",
    summary: "Site says what the work needs; it is checked against the shelf and approved before anybody buys anything.",
    body: "A material request is how site asks for material without committing any money. It names the job, the store it should go to, when it is needed, and what — and the moment it is raised it goes for approval: the site engineer, then the project manager, then procurement. Procurement sits last because it acts on the request rather than outranking it. Each request is checked against what is already on the shelf, so the card says in a sentence whether it can be issued from stock or has to be bought. Once approved, procurement either issues what is in stock from Receive & Issue, raises an enquiry to get prices, or clicks Raise an order to turn it straight into a purchase order with the lines already filled in. A request that is waiting says NOT APPROVED across its printed copy, so nobody buys against a wish.",
    steps: [
      "Open Inventory & SCM -> Material Requests and click Raise a request.",
      "Choose the job, the store it should be delivered to, and the date it is needed on site.",
      "Add a line for each item and quantity. Pick the item where it exists so the shelf can be checked; describe it where it does not.",
      "Click Send for approval. It appears in the Approvals inbox of the site engineer, then the project manager, then procurement.",
      "Read the sentence on the card: it says what is short on the shelf and what can be issued now.",
      "When it is approved, procurement clicks Raise an order, or raises an enquiry first when prices are needed.",
    ],
    tip: "Say when it is needed, not just what. A request with no date is read as whenever, and material that arrives the day after the pour is material that was not ordered.",
  },
  {
    id: "stores-orders", category: "Stores", screens: ["/inventory/orders"],
    title: "Purchase orders, and receiving against them",
    summary: "The commitment to buy: approved before anything can arrive against it, and complete only when it has actually arrived.",
    body: "A purchase order is the moment money is committed, so it is approved before it counts. Sending one for approval routes it by value: the project manager, the operations manager and a director, with the managing director added from AED 50,000. Nothing can be received against an order until that is done. When material arrives, click Record delivery on the line, and say how much came, into which store and bin, and the supplier's delivery note number. How much has arrived is added up from those deliveries rather than typed, so an order moves itself to Partly received and then Received — nobody can mark it complete when it is not. Put a promised date on every order: it is what the late-delivery count and the supplier's on-time record are measured against. An order posts nothing to the accounts. There is no VAT on it either; the tax point is the supplier's invoice.",
    steps: [
      "Open Inventory & SCM -> Purchase Orders and click Raise an order — or click Raise an order on an approved material request to arrive with its lines filled in. Awarding an enquiry raises a draft order by itself, at the winning supplier's prices.",
      "Choose the supplier, the job, the store to deliver to and the promised date. Check each price.",
      "Click Send for approval. It shows Awaiting approval until everyone on the route has signed, and nothing can be received against it until then.",
      "Once approved, click Print or Download at the bottom of the card and send the PDF to the supplier.",
      "When material arrives, click Record delivery on each line and enter the quantity, store, bin and delivery note.",
      "Items that need QA/QC then show as Pending on Receive & Issue until somebody inspects them.",
    ],
    tip: "Receive what actually came, not what was ordered. Short deliveries are normal, and the order keeps the balance open by itself.",
  },
  {
  ...
  },
  {
    ...
    body: "Job costing gets labour from attendance automatically. Material used to arrive only if somebody remembered to tag a supplier invoice to the right job by hand, which meant a job was as accurate as the last person to remember. Issuing from the store now does it instead: the cost leaves inventory and lands on that contract, priced at what the material actually cost you.",
    steps: [
      "Record the delivery when material arrives. It becomes stock, not cost.",
      "Issue it to the job when it goes out to site. That is the moment it becomes that contract's cost.",
      "If unused material comes back, record it as returned. The job is credited with what it cost.",
      "Open Finance -> Job Costing and the material is there alongside the labour.",
    ],
    tip: "The screen refuses an issue with no job on purpose. Material walking out of the store to nowhere is the easiest way for a contract to look more profitable than it really is.",
  },
  {
